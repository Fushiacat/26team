# T-004: Просмотр и редактирование анкеты
...
Реализовать хендлеры `/me` (просмотр анкеты) и `/edit` (редактирование через FSM).

## Технические детали

### 1. Просмотр анкеты /me

Файл: `src/handlers/profile.py`
...
### 2. Inline-кнопка редактирования

Файл: `src/keyboards.py` — добавить:
...
- `src/handlers/profile.py` (создание)
- `src/keyboards.py` (редактирование — добавить `edit_keyboard()`)
- `src/bot.py` (редактирование — подключить роутер)

## Критерии проверки

- [ ] `/me` показывает текущую анкету (или сообщение об отсутствии)
- [ ] Inline-кнопка "Редактировать" работает
- [ ] FSM позволяет выбрать поле для редактирования
- [ ] Редактирование имени, направления, роли, описания работает
- [ ] Данные обновляются в БД
- [ ] После редактирования FSM очищается
...
T-005: Реализовать /find и логику матчинга
